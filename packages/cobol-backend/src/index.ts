 ...
 *
 * `"00" THRU "09"` is IBM's successful-completion group and is what the first
 * character test meant. It is a range rather than a list because status key 2
 * carries a binary value for the 9x class, so a value the program has never
 * seen still falls on the failure side.
 */
const FILE_STATUS_CONDITIONS: [suffix: string, value: string][] = [
  ["OK", '"00" THRU "09"'],
  ["EOF", '"10"'],
  ["DUPKEY", '"22"'],
  ["NOTFND", '"23"'],
...
/** The status codes each condition name covers, for building a test. */
const FILE_STATUS_NAMES: Record<string, string> = {
  "10": "EOF",
  "22": "DUPKEY",
  "23": "NOTFND",
};
...
export function emitCobol(
  ...
  currentStatementNames = planStatementNames(program);
  currentSharedTableNames = planSharedTableNames(program);
  ...
  // The prologue, which is the first thing anybody reads. Column 7 is the
  // indicator area, so a comment starting in column 1 puts its fourth character
  // there and the line is rejected; `*>` in Area A is the floating comment
  // indicator and reads the same.
  for (const line of prologueLines(
    program,
    collectPrologueFacts(program, {
      programName: toCobolProgramId(program.moduleName),
      ddNameOf: toDdName,
      copybookMemberOf: copybookMemberName,
      parmFields: batchParmFields(program),
      restartedFiles: restartedFiles(program),
    }),
    {
      sourceFile: program.sourceFile,
      copybookMode: options.copybookMode ?? "inline",
    },
  )) {
    addLine(line);
  }
  ...
  if (program.files.length > 0) {
    ...
    for (const file of program.files) {
      ...
      if (file.recordVarying) {
        ...
        const varyingClauses = qsamClauses(file);
        varyingClauses.forEach((clause, index) =>
          addLine(index === varyingClauses.length - 1 ? `${clause}.` : clause),
        );
      ...
      const clauses = [...qsamClauses(file)];
      if (file.linage) {
        clauses.push(`           LINAGE IS ${file.linage.lines} LINES`);
  ...
  if (program.sql.length > 0) {
    ...
    for (const statement of program.sql) {
      ...
      // it whatever the database keeps returning. It is the program's own
  ...
  const sqlResultRecords = new Set(
    program.sql.flatMap((statement) =>
      statement.resultRecordName ? [statement.resultRecordName] : [],
    ),
  );

  for (const record of program.records) {
    const isHostRecord = sqlResultRecords.has(record.name);
    if (isHostRecord) {
      addLine(`           EXEC SQL BEGIN DECLARE SECTION END-EXEC.`);
    }
  ...
  if (program.sql.length > 0) {
    ...
    // so it belongs outside a declare section.
...
/**
 * The two clauses a QSAM file description carries and a VSAM one does not.
 *
 * `BLOCK CONTAINS 0 RECORDS` asks for a system-determined block size, which the
 * Programming Guide names as the way to get one: "code BLOCK CONTAINS 0 in your
 * source program ... do not code a BLKSIZE value in the JCL DD statement". The
 * generated job writes `BLKSIZE=0` for the same reason. Without either, the
 * file defaults to unblocked, and an unblocked dataset of short records wastes
 * most of a track.
 *
 ...
 * Reference says BLOCK CONTAINS "can be omitted when the associated
 * file-control entry specifies a VSAM file".
 */
function qsamClauses(file: IRFile): string[] {
  if (file.organization !== "sequential") {
    return [];
  }
  return [
    "           BLOCK CONTAINS 0 RECORDS",
    `           RECORDING MODE IS ${file.recordVarying ? "V" : "F"}`,
  ];
}
...
function ioFailed(status: string, expected: string[]): string {
  return [
    `NOT ${cobolWord(status, "OK")}`,
    ...expected.map((code) =>
      FILE_STATUS_NAMES[code]
        ? `NOT ${cobolWord(status, FILE_STATUS_NAMES[code])}`
        : `${status} NOT = "${code}"`,
    ),
  ].join(" AND ");
}
...
function emitSearchStatement(
  ...
  const index = tableIndexName(
    statement.arrayFieldName,
    toCobolName(statement.arrayRecordName),
  );
 ...
 * belonged to.
 *
 * Keyed by statement identity, planned once per program before anything is
 * emitted, so the same statement gets the same name wherever it is reached
 * from.
 */
let currentStatementNames = new Map<object, string>();

function planStatementNames(program: IRProgram): Map<object, string> {
  ...
  const walk = (
    ...
    for (const statement of block.statements) {
      if (statement.kind === "WhileStatement") {
        names.set(statement, cobolWord(routine, "LOOP", String(next("loop"))));
      }
  ...
  for (const transaction of program.transactions) {
    const routine = toCobolName(transaction.name);
    walk(transaction.body, routine, new Map());
    if (transaction.failureHandler) {
      walk(transaction.failureHandler, routine, new Map());
    }
  }
  for (const fn of program.functions) {
    walk(fn.body, toCobolName(fn.name), new Map());
  }
...
function statementName(statement: object, fallback: string): string {
  return currentStatementNames.get(statement) ?? fallback;
}

function loopCounterName(statement: IRWhileStatement): string {
  return statementName(statement, "BANK-LOOP");
}
...
function emitComputeInto(
  ...
  // needs `ON SIZE ERROR`: an expression that cannot overflow is one with no
  // arithmetic in it, which is a reference, a literal, or a routine's result
  // cell. Everything else stays a COMPUTE and keeps its guard.
  if (!canSizeError(expression)) {
    addLine(
      `${indent}MOVE ${renderDecimalExpression(expression)} TO ${target}`,
    );
    return;
  }
 ...
 * loop, from the routine holding the SORT and its ordinal within it.
 */
function sortProcedureName(
  ...
  return cobolWord(
    statementName(statement, "BANK-SORT"),
    kind === "input" ? "IN" : "OUT",
  );
}
...
function tableIndexName(fieldName: string, recordName?: string | null): string {
  const bare = toCobolFieldName(fieldName);
  const qualify = currentSharedTableNames.has(bare);
  const group = recordName ?? currentGroupName;
  return qualify && group
    ? cobolWord(group, bare, "IDX")
    : cobolWord(bare, "IDX");
}

/** Table field names declared by more than one record, so an index collides. */
let currentSharedTableNames = new Set<string>();

function planSharedTableNames(program: IRProgram): Set<string> {
  const counts = new Map<string, number>();

  const walk = (fields: IRRecord["fields"], seen: Set<string>): void => {
    for (const field of fields) {
      if (field.type.kind === "array") {
        seen.add(toCobolFieldName(field.name));
        if (field.type.element.kind === "record") {
          walk(field.type.element.fields, seen);
        }
        if (field.type.element.kind === "array") {
          seen.add(toCobolFieldName(innerTableName(field.name)));
        }
      }
      if (field.type.kind === "record") {
        walk(field.type.fields, seen);
      }
    }
  };
  ...
  for (const record of [
    ...program.records,
    ...program.files.map((file) => file.record),
  ]) {
    const seen = new Set<string>();
    walk(record.fields, seen);
