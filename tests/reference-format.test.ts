...
describe("across the corpus", () => {
   ...
   * comment.
   *
   * z/OS does. Source reaches the mainframe in EBCDIC, and U+2014 has no
   * mapping in the single-byte code pages this compiler targets: the transfer
   * either substitutes it or delivers three bytes where the page counts one, in
   * a format whose columns are counted in bytes and whose margin is absolute.
   * `docs/status-and-limits.md` says the character model is one byte in the
   * host code page, and the compiler's own output was the first thing breaking
   * that.
   *
   * Held over the copybooks and the JCL too. A copybook is `COPY`d into the
   * program and the JCL is submitted to JES, and neither is any more tolerant.
   */
  it("emits nothing but ASCII, in any artifact", () => {
    let bytes = 0;
    for (const example of EXAMPLES) {
      const { ir } = compileExample(example);
      if (!ir.program) {
      ...
      const artifacts: [string, string][] = [
        ["cobol", emit.cobol],
        ["jcl", emitJcl(ir.program).jcl],
        ...ir.program.records.map((record): [string, string] => [
          `copybook ${record.name}`,
          renderCopybook(record),
        ]),
      ];

      for (const [what, text] of artifacts) {
        bytes += text.length;
        const outside = [...text]
          .map((character, at) => ({ character, at }))
          .filter((entry) => entry.character.charCodeAt(0) > 0x7f)
          .map(
            (entry) =>
              `${example} ${what} at ${String(entry.at)}: ${JSON.stringify(entry.character)} (U+${entry.character
                .charCodeAt(0)
                .toString(16)
                .toUpperCase()
                .padStart(4, "0")})`,
    ...
    checked(bytes, 100_000, "generated bytes");
  });
