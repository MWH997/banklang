...
describe("record arguments", () => {
  /**
   * A record parameter binds to the record's group item, so the callee reads
   * that group whatever the caller passed. Anything but a plain name would
   * compile and silently read different storage.
   */
  it("rejects a record argument that is not a plain name", () => {
    const result = compile(`${PREAMBLE}
record Inner {
  amount: BDT;
}
...
transaction sum(inner: Inner) {
  ...
  audit("SUMMED", inner.idempotencyKey);
    ...
    expect(result.diagnostics).toEqual([]);
    // The callee reads the group, so there is no parameter field to move into.
    expect(result.cobol).not.toContain("TOTAL-P1");
  });
});
