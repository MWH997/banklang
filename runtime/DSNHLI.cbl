      ...
      *> one line per outcome it wants, as
      ...
      *>     0004 -911 40001 0000   statement 4 is rolled back by a deadlock
      *>
      *> A count of 0000 means every remaining call, which is what a statement
      *> run once wants. A finite count is what a FETCH inside a cursor loop
      *> wants: the loop ends when a fetch stops returning zero, so the number of
      *> rows it processes is the number of successes scripted here.
      *>
      *> Lines are matched in file order, so the first entry for a statement that
      *> still has calls left wins. A statement the file does not mention
      *> succeeds, and with no file at all every statement succeeds.
      *>
      ...
      *> selects — so a not-found path, and the end of a cursor, are executed
      *> rather than assumed.
      *>
      ...
      *>
      *> Host variables are never written. They are passed, so the COBOL compiler
      *> checks that each one resolves, but this program has no way to know their
      *> types or lengths and does not touch them. A fetched row therefore arrives
      *> unchanged: a test can assert how many rows a loop processed and that it
      *> opened, bounded, and closed correctly, but not what was in them.
      ...
      *> Calls this entry still applies to. -1 means every remaining call.
      ...
      *> First entry for this statement that still has calls left. A finite
      *> entry is spent as it is used, so a later entry for the same statement
      *> takes over — which is how a cursor runs out of rows.
       APPLY-OUTCOME.
      ...
      *> An omitted or zero count means every remaining call, so a file written
      *> before counts existed still means what it meant.
                   IF WS-PARSED-TIMES = SPACES OR WS-PARSED-TIMES = "0000"
