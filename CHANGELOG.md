...
- Added record inheritance with `extends`. Base fields are laid out first, so a derived record's leading storage is the base record's storage byte for byte and an existing copybook for the base still reads a derived record. Inheritance is deliberately layout-only: a record parameter binds to a group item in working storage, so passing a derived record where the base is expected would read the wrong storage and is rejected (`BANK-TYPE-021`). Cycles are `BANK-TYPE-016` and redeclared inherited fields are `BANK-TYPE-017`.
...
- Passing a record to a function generated a `MOVE` into a `-Pn` field that was never declared, so the program did not compile. Record parameters bind to the record's group item, so no move is needed; an argument that is not a plain record name is now `BANK-TYPE-021` rather than silently reading a different group.
