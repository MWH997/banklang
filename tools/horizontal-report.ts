...
function renderDefectMatrix(rows: DefectRow[]): string {
  ...
  return `${[
    ...
    ...rows.map(
      (row) =>
        `| ${row.defect} | ${row.family} | ${row.title.toLowerCase()} | ${row.coverage} | ${row.diagnostic ? `\`${row.diagnostic}\`` : "none"} |`,
    ),
    "",
...
function renderResults(cwd: string): string {
  ...
  if (environment) {
    lines.push(
      ...
      `| COBOL compiler | ${environment.gnucobolVersion ?? "not available, so nothing was executed"} |`,
  ...
  for (const id of ["cobolcodebench", "coboleval"]) {
    ...
    lines.push(
      ...
      `| applicable, meaning BankTS can express it | ${String(summary.applicable)} |`,
