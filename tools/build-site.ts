 ...
 * this file, and `tests/site-layout.test.ts` holds its header to the generated
 ...
 * The site header.
 *
 * One header for every page, written here and used by the landing page, the
 * 404, every documentation page and every post. It replaces five hand-kept
 * copies that had already drifted: the playground's listed Language and
...
export type NavSection = "docs" | "blog" | "playground";

export interface HeaderOptions {
  /** What reaches the site root from the page being written: "/" or "../". */
  up: string;
  /** The section this page belongs to, marked with `aria-current`. */
  current?: NavSection | null;
  /**
   * A control that belongs beside the menu button rather than inside the menu.
   *
   * The documentation's sidebar toggle is the only one. It opens the page's own
   * contents rather than the site's navigation, and putting it inside the menu
   * would have made reaching a heading on the current page take two taps
   * through a control that says "Menu".
   */
  extra?: string;
}
...
interface NavItem {
  ...
  /** Appended to `up`, or used as written when it names another site. */
  href: string;
  label: string;
  external?: boolean;
}
...
const NAV_ITEMS: NavItem[] = [
  ...
  { section: "blog", href: "blog/", label: "Writing" },
  { section: "playground", href: "playground/", label: "Playground" },
  {
    section: null,
    href: "https://github.com/MWH997/banklang",
    label: "GitHub",
    external: true,
  },
  {
...
export function siteHeader(options: HeaderOptions): string {
  const links = NAV_ITEMS.map((item) => {
    const href = item.external ? item.href : `${options.up}${item.href}`;
    const attributes = [
      `href="${href}"`,
      item.external ? 'rel="noopener"' : "",
      item.section && item.section === options.current
        ? 'aria-current="page"'
        : "",
    ]
      .filter(Boolean)
...
${options.extra ? `${options.extra}\n` : ""}          <button class="burger" id="nav-burger" type="button" aria-expanded="false" aria-controls="site-nav">
            <span class="burger__box" aria-hidden="true"></span>
            <span class="burger__label">Menu</span>
          </button>
        </div>
      </div>
      <nav class="top__nav" id="site-nav" aria-label="Main">
...
/**
 * The menu button's behaviour.
 *
 * Escape closes and puts focus back on the button, because a menu that can be
 * opened from the keyboard and only closed with a pointer is one a keyboard
 * user is stuck inside. A click outside closes it for the same reason a pointer
 * user expects it to. Crossing the breakpoint closes it and clears the
 * attribute, so a menu opened on a phone and then rotated does not leave the
 * button claiming to be expanded while the links are laid out along the header.
 *
 * The button is written into the page already collapsed, so this adds no state
 * that a reader with no JavaScript is then stuck in: the stylesheet shows the
 * links whenever `.top` does not carry `open`, and the whole control is hidden
...
export function siteContent(): SiteContent {
  ...
  return {
    ...
    // holds fewer directories than programs, because one of them is a job of
    // several; counting directories for the denominator and programs for the
    // numerator read as "21 of 21 are executed" when two were not.
...
export function render404(): string {
  // The two shared blocks are filled in before the scripts are lifted, because
  // that is what makes them the same bytes here as on the landing page and so
  // covered by the same two hashes.
  const template = readFileSync(join(SITE, "index.html"), "utf8")
    .replaceAll("{{THEME_SCRIPT}}", THEME_SCRIPT)
    .replaceAll("{{NAV_SCRIPT}}", NAV_SCRIPT);
